...
showScaleIn = False
showScaleOut = False
showPlusPressed = False
showMinusPRessed = False
...
def draw_vmss():
   vmss_coords = dc_map[vmssProperties[2]] # locating VMSS on world map
   ...
   draw_text(vmssProperties[0], label_coords, 14)
   draw_text('Name: ' + vmssProperties[0], [msg_xstart, msg_ystart], 28)

   # draw capacity
   draw_text('VMs: ' + str(vmssProperties[1]), [msg_xstart, msg_ystart + 30], 28)
   minus_coords = [msg_xstart + 110, msg_ystart + 30]
   plus_coords = [msg_xstart + 170, msg_ystart + 30]
   if showScaleIn == True:
   ...
   # draw sku
   draw_text('OS: ' + vmssProperties[4] + ' ' + vmssProperties[5], [msg_xstart, msg_ystart + 80], 28)

   # draw dns details 
   draw_text(vmssProperties[7] + ' (' + vmssProperties[8] + ')', [msg_xstart, msg_ystart + 140], 28)
   
   # draw provisioning state
   draw_text('Provisioning state: ' + vmssProperties[6], [msg_xstart, msg_ystart + 170], 28)
...
def process_click(clickpos, access_token):
   ...
   if clickpos[0] > msg_xstart + 110 and clickpos[0] < msg_xstart + 150 and clickpos[1] > msg_ystart + 30 and clickpos[1] < msg_ystart + 60:
      showScaleIn = True
      clickLoopCount = 10
      scale_thread = threading.Thread(target=scale_event, args=(-1, access_token))
      scale_thread.start()
   # check if user clicked on plus sign
   elif clickpos[0] > msg_xstart + 170 and clickpos[0] < msg_xstart + 200 and clickpos[1] > msg_ystart + 30 and clickpos[1] < msg_ystart + 60:
      showScaleOut = True
      clickLoopCount = 10
      scale_thread = threading.Thread(target=scale_event, args=(1, access_token))
      scale_thread.start()
   # check if user clicked on DNS addr of load balancer IP and launch a browser tab
   elif clickpos[0] > msg_xstart and clickpos[0] < msg_xstart + 900 and clickpos[1] > msg_ystart + 145 and clickpos[1] < msg_ystart + 170:
